...
class DirectionCreateSerializer(ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        print(obj.id)
        return obj kljhgfhxdgcjvb
        
        # to_role = self.validated_data.get('to_role'),
        # imams = User.objects.filter(role=to_role[0])
        # imam_list = self.validated_data.get('to_imams', None)
        # district_list = self.validated_data.get('to_district', None)
        # region_list = self.validated_data.get('to_region', None)
        # for i in imams:
