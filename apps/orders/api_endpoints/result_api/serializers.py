...
class DirectionsEmployeeResultListSerializer(ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'direction', 'employee', 'comment', 'files', 'images', 'videos', 'created_at',)


class DirectionsEmployeeResultSerializer(ModelSerializer):
    class Meta:
        model = DirectionsEmployeeResult
        fields = ('id', 'direction', 'employee', 'comment', 'files', 'images', 'videos',)
    
    def validate(self, attrs):
        direction_date = Directions.objects.get(id=attrs.get('direction').id).to_date
    ...
    def create(self, validated_data):
        ...
        result.save()
        DirectionsEmployeeRead.objects.filter(direction=result.direction, employee=self.context['request'].user.id).update(state="3")
